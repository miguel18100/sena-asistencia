...
function pintarQR(codigo) {
  ...
  // El QR codifica una URL directa a la pantalla de escaneo del aprendiz,
  // con el código de sesión ya incluido (?codigo=XXXXXX). Así, si el
  // aprendiz lo captura con la cámara nativa del celular (no con el
  // escáner interno de la app), el enlace lo redirige automáticamente
  // a "escanear-qr.html" y esa pantalla completa el registro sola.
  const urlEscaneo = construirUrlEscaneo(codigo);
...
function construirUrlEscaneo(codigo) {
  // Esta página vive en /pages/codigo-qr.html, así que escanear-qr.html
  // está en la misma carpeta.
  const base = window.location.href.split("?")[0].replace(/codigo-qr\.html$/, "escanear-qr.html");
